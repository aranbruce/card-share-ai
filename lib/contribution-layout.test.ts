...
describe("contribution-layout", () => {
  it("coerces string layout numbers from the API", () => {
    ...
    expect(contributionHasCanvasPosition({ position_x: "12", position_y: "8" })).toBe(
      true,
    )
  ...
  it("treats missing positions as not placed", () => {
    expect(contributionHasCanvasPosition({ position_x: null, position_y: null })).toBe(
      false,
    )
    expect(
      contributionHasCanvasPosition({ position_x: undefined, position_y: undefined }),
    ).toBe(false)
  })
})

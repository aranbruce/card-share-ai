...
- **Creating test users via Supabase Admin API**: You can create and email-confirm a test user with `POST /v1/signup` then `PUT /v1/admin/users/{id}` with `{"email_confirm":true}` using the service role key. Set `E2E_EMAIL`/`E2E_PASSWORD` to those credentials for authenticated E2E tests.

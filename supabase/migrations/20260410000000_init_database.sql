-- Baseline schema: cards + card_contributions + extra_pages, minus copy_signoff.
-- Represents the DB state before the numbered supabase/migrations were introduced.
...
CREATE POLICY "Public can view cards with contributor link" ON cards
  FOR SELECT USING (contributor_link_id IS NOT NULL);
...
CREATE POLICY "Anyone can view contributions" ON card_contributions
  FOR SELECT USING (true);

CREATE POLICY "Anyone can add contributions" ON card_contributions
